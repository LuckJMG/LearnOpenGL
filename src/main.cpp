...
int main() {
	...
	Shader singleColorShader("shaders/test_vertex.glsl", "shaders/singleColorFragment.glsl");
	...
	while(!glfwWindowShouldClose(window)) {
		...
		singleColorShader.set("view", view);
		singleColorShader.set("projection", projection);
		...
		singleColorShader.set("model", model);
		glDrawArrays(GL_TRIANGLES, 0, 36);
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(2.0f, 0.0f, 0.0f));
		model = glm::scale(model, glm::vec3(scale, scale, scale));
